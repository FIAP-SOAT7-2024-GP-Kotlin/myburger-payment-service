...
class PaymentIntegrationClient(
    ...
    override fun requestQRCodeDataForPayment(payment: Payment): QRCodeData {
        try {
            ...
            val paymentToRequest = payment.toPaymentRequest(notificationUrl)

            logger.info { "Requesting PaymentData with [payload: $paymentToRequest]" }

            val response = paymentRestTemplate.exchange(
                paymentServiceUrl,
                HttpMethod.POST,
                HttpEntity(paymentToRequest, headers),
